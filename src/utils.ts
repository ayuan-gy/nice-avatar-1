...
function brightenColor(color: string, amount: number): string {
  ...
  const brightenedRgb: number[] = rgb.map((value) =>
    Math.min(value + amount, 255)
  );
...
class Color {
  ...
  brighten(amount: number): this {
    this.color = brightenColor(this.color, amount);
    return this;
  }
...
// 创建自定义颜色对象
export function chroma(color: string): Color {
  return new Color(color);
}

// 转换可用于 background-image
